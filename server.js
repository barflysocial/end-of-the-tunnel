...
function servePlayerWithRichPreview(req, res) {
  ...
  const imageUrl = `${origin}/assets/end-of-the-tunnel-title-bg.png`;
...
function getPack(sessionOrId) {
  ...
  return truthPacks[id] || truthPacks['end-of-the-tunnel-rookie'] || truthPacks['end-of-the-tunnel'] || truthPacks.demo;
}
...
function createSession({ tableName = 'Table 1', truthPackId = 'end-of-the-tunnel-rookie', playerCap = 25, eventDate = '', eventTime = '', eventDurationMinutes = 45, eventStatus = 'open', eventType = 'paid', ticketPrice = '', eventPrice = '', sharedAccessCode = '', allowLateCheckIn = false, scheduledStartAt = 0, autoStartEnabled = true } = {}) {
  let code = makeCode();
  ...
  const session = {
    ...
    code,
    tableName,
    truthPackId: truthPacks[truthPackId] ? truthPackId : 'end-of-the-tunnel-rookie',
...
function prepareDemoSession(session) {
  ...
  session.tableName = session.tableName || 'Training Level 1 — The Tunnel';
  session.truthPackId = 'end-of-the-tunnel-training';
  ...
  if (!session.hostMessages.length) {
    ...
    addHostMessage(session, {
      ...
      text: pack.openingNarration || 'The End of the Tunnel Training Level 1 is live. Review the evidence as it unlocks and submit your accusation before time runs out.',
      kind: 'opening'
    });
  }
  return session;
...
function getOrCreateDemoSession() {
  ...
  if (!session) {
    session = createSession({
      tableName: 'Training Level 1 — The Tunnel',
      truthPackId: 'end-of-the-tunnel-training',
      playerCap: 100,
      eventStatus: 'private',
...
app.get('/api/truth-packs', (_req, res) => {
  ...
  const packs = Object.values(truthPacks)
    .filter(p => p.gameId === 'end-of-the-tunnel' && p.id !== 'end-of-the-tunnel')
    .sort((a, b) => order.indexOf(a.levelId) - order.indexOf(b.levelId));
  res.json(packs.map(p => ({
    id: p.id,
    gameId: p.gameId || 'end-of-the-tunnel',
    gameTitle: p.gameTitle || 'The End of the Tunnel',
    levelId: p.levelId || '',
  ...
  const items = Array.from(sessions.values())
    ...
    .map(session => {
      ...
      return {
        sessionCode: session.code,
        tableName: session.tableName,
        mystery: pack.gameTitle || 'The End of the Tunnel',
        mysteryTitle: pack.title || pack.gameTitle || 'The End of the Tunnel',
        venue: pack.venue || 'The Tunnel • Baton Rouge, Louisiana',
