...
const BARFLY_APP_URL = 'https://end-of-the-tunnel.onrender.com/player/';
  ...
  if (navigator.share) {
    try {
      await navigator.share({
        ...
        text: 'RSVP or join The End of the Tunnel: A Live Detective Mystery Experience.',
        url: shareUrl
      });
...
function updateLevelLabels(s = state) {
  ...
  if ($('topbarSubtitle')) $('topbarSubtitle').textContent = `Barfly Social Presents · The Tunnel${diff ? ` · ${diff}` : ''}`;
}
...
function showtimeCardHtml(item) {
  ...
  return `<article class="showtimeCard">
    <div>
      ...
      <h4>${escapeHtml(item.mysteryTitle || item.mystery || 'The End of the Tunnel')}</h4>
      <p>${escapeHtml(item.levelLabel || item.difficultyLabel || item.difficulty || 'Skill level TBD')} · ${escapeHtml(item.venue || 'The Tunnel')}</p>
...
function selectRsvpSession(code) {
  ...
  $('selectedSessionCard').innerHTML = `<div class="time">Selected Showtime</div>
    <h3>${escapeHtml(item.mysteryTitle || item.mystery || 'The End of the Tunnel')}</h3>
    <p><b>${escapeHtml(item.dateLabel || 'Date TBD')} · ${escapeHtml(item.timeLabel || 'Time TBD')}</b></p>
    <p>${escapeHtml(item.levelLabel || item.difficultyLabel || item.difficulty || 'Skill Level TBD')} · ${escapeHtml(item.venue || 'The Tunnel • Baton Rouge, Louisiana')}</p>
  ...
  try {
    ...
    if (result) {
      ...
      result.innerHTML = `
        ...
        <p><b>Session:</b> ${escapeHtml(session.tableName || session.truthPackTitle || 'The End of the Tunnel')}</p>
...
const STORY_BRIEFINGS = {
  training: [
    ['0:00–0:45','Peacock Alley','The End of the Tunnel begins at The Tunnel in Baton Rouge, inside a speakeasy tied to the old passage between the Heidelberg and King hotels.','The tunnel was built for service and discretion. Tonight it becomes a crime scene.'],
    ['0:45–1:30','The Victim','Dr. Malcolm Vire arrived with a hotel ledger, a King Hotel key, and a sealed envelope marked DO NOT DELIVER.','He believed the tunnel had been opened recently, and someone wanted him stopped.'],
    ['1:30–2:15','The First Problem','The lights went out during the historical preview. When they returned, Malcolm was dead near the sealed tunnel door.','Focus first on access: who could reach the door during the blackout?'],
    ['2:15–3:30','People Under Pressure','Vivian Cross owns the room. Julian March controls the event. Celeste Moreau knows the rumors. Rhett Boudreaux handled renovations. Nora Vale found the archive. Bennett Clay buys political artifacts. Mara King protects the King Hotel name.','Motive alone is not enough. Match motive to access.'],
    ['3:30–4:15','Evidence Path','Your apps will unlock calls, messages, maps, ledger files, photos, social clips, contacts, notes, and browser evidence.','The old Huey Long rumor may matter, but it may also be bait.'],
    ['4:15–5:00','Your Mission','Answer each checkpoint as the rounds close, then submit the final accusation at the end.','Do not chase the loudest legend. Follow the evidence that survives comparison.']
  ],
  rookie: [
    ['0:00–0:45','A Legend Reopens','Guests came for a preview about Peacock Alley, the Heidelberg, the King Hotel, and the famous political escape stories. Then the historian died.','Rookie level gives you a clear path through the blackout, the door, and the access evidence.'],
    ['0:45–1:30','Malcolm Knew Too Much','Malcolm had proof that a supposedly sealed tunnel showed signs of recent use.','Someone was using the past as cover for a present-day secret.'],
    ['1:30–2:15','The Door and the Dust','A moved rope, fresh dust, and a service-panel contradiction create the first serious clue chain.','If the tunnel was sealed, the evidence should not look fresh.'],
    ['2:15–3:30','Suspects With Motives','The owner, event director, singer, contractor, archivist, collector, and descendant all have reasons to control the story.','The right suspect must explain method, access, and motive.'],
    ['3:30–4:15','What to Compare','Compare phone movement, access records, photos, and file metadata. A single clue may look weak alone, but together they form a chain.','The right answer should explain both the murder and the cover story.'],
    ['4:15–5:00','Your Mission','Do not submit based on suspicion. Submit based on the chain.','The tunnel was once used for escape. Tonight it was used for access.']
  ],
  junior: [
    ['0:00–0:45','Archive Trouble','At Junior Detective level, the ledger and hotel records become more important than the blackout alone.','A missing page may reveal more about today than the 1920s.'],
    ['0:45–1:30','A Family Name','The King Hotel story is valuable, embarrassing, and easy to manipulate.','Ask who benefits if the archive says one thing and the evidence says another.'],
    ['1:30–2:15','The Forged Trail','Ink, fibers, gloves, and a key stamp create a path through the files.','Junior level asks you to compare paper evidence with physical access.'],
    ['2:15–3:30','The Red Herrings','The Huey Long legend draws attention, but legends can be edited by whoever controls the documents.','A rumor can hide a forgery.'],
    ['3:30–4:15','What to Prove','Look for the evidence that connects motive, location, and method.','The right final answer is not just who had a reason; it is who shaped the record.'],
    ['4:15–5:00','Your Mission','Build the timeline twice: once from what people claimed, and once from what the records prove.','The difference between those timelines is where the killer stands.']
  ],
  detective: [
    ['0:00–0:45','Artifacts and Alibis','At Detective level, political memorabilia, camera metadata, and antique documents become central.','The killer used history as a stage prop.'],
    ['0:45–1:30','A Clean Story','One suspect wants the room focused on Huey Long gossip while the real evidence points to modern forgery.','When proof looks antique, test whether it was made yesterday.'],
    ['1:30–2:15','The Hidden Preparation','UV ink, edited files, and collector payments create a path that is easy to overlook if you only chase the tunnel myth.','Detective level rewards players who test the documents.'],
    ['2:15–3:30','Suspect Pressure','Everyone has a reason to care about Peacock Alley, but not everyone profits from a forged version of the past.','This level asks who could build a profitable lie and kill to protect it.'],
    ['3:30–4:15','What to Compare','Compare payment trails, document tests, camera exports, and suspect statements. Small technical facts matter.','If a clue explains timing, preparation, and opportunity, it belongs near the solution.'],
    ['4:15–5:00','Your Mission','Do not solve the level like a simple tunnel accident. Treat the legend itself as part of the weapon.','The killer did not only act in the room. The killer controlled the story.']
  ],
  senior: [
    ['0:00–0:45','A Secret Inside the Secret','Senior Detective level assumes the killer knows investigators will follow the famous rumor.','This level is about the blackmail system behind the legend.'],
    ['0:45–1:30','The King Hotel Side','A key, a sealed envelope, and a drop schedule create a deeper pattern than one dramatic blackout.','Access records can lie when the wrong person has the right key.'],
    ['1:30–2:15','Money Behind the Curtain','Blackmail drops, family pressure, and rewritten hotel history give one suspect a reason to silence Malcolm.','Senior level makes the motive harder because the old rumor is deliberately louder than the modern crime.'],
    ['2:15–3:30','Noise and Misdirection','Every suspect looks connected to the tunnel. That does not mean every clue points to the solution.','Ask which suspect benefits from everyone arguing about the past.'],
    ['3:30–4:15','What to Prove','The strongest solution must explain the envelope, the King Hotel key, the access window, and the blackmail schedule.','One answer should connect all four without coincidence.'],
    ['4:15–5:00','Your Mission','Attack every easy conclusion. If a clue points too directly at history, ask who placed it there.','Senior Detective cases are solved by finding the person who controlled the secret after the crime.']
  ]
};
...
function renderDetectiveNotes(options = {}) {
  ...
  $('appEvidence').innerHTML = `<div class="detectiveNotesPanel">
    ...
    <div class="notesGrid">
      ...
      ${noteField('Possible Motive', 'possibleMotive', 'Why would they need Malcolm stopped?')}
...
async function renderBadgeCanvas(result) {
  ...
  const bg = await loadImage('/assets/end-of-the-tunnel-title-bg.png');
  ...
  ctx.fillText('The End of the Tunnel', centerX, 1398);
  ctx.fillStyle = '#ffd7f4';
  ctx.font = '700 31px Arial';
  ctx.fillText('The Tunnel', centerX, 1454);
...
async function shareBadge() {
  ...
  const file = new File([blob], `end-of-the-tunnel-${safeName}.png`, { type: 'image/png' });
  try {
    if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
      await navigator.share({ title: 'The End of the Tunnel', text: `${result.playerName} earned the ${result.badge} badge.`, files: [file] });
    } else {
      await downloadBadge();
    }
  } catch (_err) {}
...
async function downloadBadge() {
  ...
  link.download = `end-of-the-tunnel-${safeName}.png`;
  link.click();
}
